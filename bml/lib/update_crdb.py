...
def insert_values_db(config, uuid, test, osp_name, avg_runtime, grade, time_stamp):  # noqa
    ...
    cur = conn.cursor()
    cur.execute("CREATE TABLE IF NOT EXISTS grades_values (uuid STRING, test STRING, osp_name STRING, avg_runtime FLOAT, grade INT , timestamp STRING, PRIMARY KEY (uuid, test, osp_name, avg_runtime))")  # noqa
