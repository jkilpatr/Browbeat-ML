    ...
    def _set_hardware_metadata(self, hardware_details):
        ...
        for node in hardware_details:
            if 'undercloud' in node['label'] and not len(self.undercloud):
                self.undercloud['machine_make'] = self._typecheck_string(node['machine_make'])
                self.undercloud['processor_type'] = self._typecheck_string(node['processor_type'])
                self.undercloud['mem'] = self._typecheck_num(node['total_mem'])
                self.undercloud['cores'] = self._typecheck_num(node['total_logical_cores'])
            elif 'controller' in node['label'] and not len(self.controller):
                self.controller['machine_make'] = self._typecheck_string(node['machine_make'])
                self.controller['processor_type'] = self._typecheck_string(node['processor_type'])
                self.controller['mem'] = self._typecheck_num(node['total_mem'])
                self.controller['cores'] = self._typecheck_num(node['total_logical_cores'])
            elif 'compute' in node['label'] and not len(self.compute):
                self.compute['machine_make'] = self._typecheck_string(node['machine_make'])
                self.compute['processor_type'] = self._typecheck_string(node['processor_type'])
                self.compute['mem'] = self._typecheck_num(node['total_mem'])
                self.compute['cores'] = self._typecheck_num(node['total_logical_cores'])
